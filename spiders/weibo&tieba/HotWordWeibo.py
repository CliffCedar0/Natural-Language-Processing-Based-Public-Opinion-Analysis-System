...
try:
    with open('../Outcome/weiboDetail.csv', 'r', encoding='utf-8') as f:
        reader=csv.reader(f)
        content= [row[4] for row in reader]     #获取评论里的每一条内容
except:
    print("数据文件不存在！")
    exit()
...
resualt['word']=key
resualt['count']=value
resualt.to_csv('../Outcome/hotWordWeibo.csv')

#保存热词并排序
data=pd.read_csv('../Outcome/hotWordWeibo.csv', encoding='utf-8')
sorted_data=data.sort_values(by=data.columns[2],ascending=False)
sorted_data=sorted_data.iloc[:,1:] #去掉第一列的无用数据
sorted_data.to_csv('../Outcome/hotWordWeibo.csv',index=False,encoding='utf-8')